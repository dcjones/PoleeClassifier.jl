...
function rand_gamma1s(αs::Array)
    n = length(αs)
    us = Array{Float64}(undef, size(αs))
    Threads.@threads for i in 1:n
        us[i] = rand_gamma1(Float64(αs[i]))
    end
    return us
end
...
ZygoteRules.@adjoint function rand_gamma1s(αs::Array{T}) where {T}
    xs = rand_gamma1s(αs)

    function rand_gamma1s_pullback(x̄)
        n = length(xs)
        ∂αs = Array{T}(undef, size(xs))
        Threads.@threads for i in 1:n
            dα, dy = gradient(
